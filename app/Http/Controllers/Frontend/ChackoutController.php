...
class ChackoutController extends Controller
{
    ...
    public function Checkout()
    {
        ...
        return view('frontend.cart.chackout',compact('content'));
    ...
    public function ApplyCoupon(Request $request)
    {
        ...
        if ($check) {
            ...
            if (date('Y-m-d', strtotime(date('Y-m-d'))) <= date('Y-m-d', strtotime($check->valid_date))) {
                if (is_numeric($check->coupon_amount)) {
                    $subtotal =(float)Cart::subtotal()-(float)$check->coupon_amount;
                    // echo $subtotal;
                  } else {
                ...
                Session::put('coupon',[
                    ...
                    'after_discount'=>(float)Cart::subtotal()-(float)$check->coupon_amount ,
                    dd( (Double)Cart::subtotal()-(Double)$check->coupon_amount),
